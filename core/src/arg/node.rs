...
use crate::types::{Env, InfonId, ModalType, NodeId, TRDId};
...
mod tests {
    ...
    fn context_mask_match_uses_bitwise_subset() {
        let n = make_node(2);
        assert!(n.matches_context_mask(0b01));
        assert!(n.matches_context_mask(0b11));
        assert!(!n.matches_context_mask(0b100));
    }
}
