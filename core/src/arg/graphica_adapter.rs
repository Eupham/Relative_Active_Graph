...
use crate::arg::{edge::ArgEdge, node::ArgNode};
use crate::types::{CacheKey, EdgeId, Env, ModalMode, NodeId, TypeCategory};
...
fn dominant_type_category(nodes: &[&ArgNode]) -> TypeCategory {
    ...
    for n in nodes {
        *counts.entry(n.mtlg_type.category.id()).or_insert(0) += 1;
    }
    counts
        .into_iter()
        .max_by_key(|&(_, c)| c)
...
impl GraphicaCache {
    ...
    pub fn try_emit_shortcut(
        ...
        src: NodeId,
        dst: NodeId,
        canonical_mode: ModalMode,
        path_weights: &[f32],
        path_edge_ids: &[EdgeId],
        next_edge_id: EdgeId,
    ) -> Option<ArgEdge> {
        use crate::arg::edge::{ArgEdge, EdgeClass};
        const MIN_SHORTCUT_TRAVERSALS: u32 = 10;
        let result = self.store.get_mut(key)?;
        if result.traversal_count < MIN_SHORTCUT_TRAVERSALS {
            return None;
        }
        if result.shortcut_canonical_id.is_some() {
            return None;
        }
        let canonical_id = key.hash ^ (canonical_mode as u64).wrapping_mul(0x9e37_79b9_7f4a_7c15);
        result.shortcut_canonical_id = Some(canonical_id);
        self.condensed_paths
            .insert(canonical_id, path_edge_ids.to_vec());
        let avg = if path_weights.is_empty() {
            0.5
        } else {
            path_weights.iter().sum::<f32>() / path_weights.len() as f32
        };
        let mut edge = ArgEdge::new(next_edge_id, src, dst, EdgeClass::DEFAULT, canonical_mode);
        edge.weight = avg.clamp(0.01, 1.0);
...
mod tests {
    ...
    use crate::types::{ModalMode, ModalType, TypeCategory};
    ...
    fn shortcut_emitted_after_threshold() {
        ...
        for _ in 0..9 {
            ...
            assert!(cache
                .try_emit_shortcut(&key, 1, 2, ModalMode::Diamond, &[0.8], &[10], 99)
                .is_none());
        }
        cache.record_traversal(&key);
        let sc = cache.try_emit_shortcut(&key, 1, 2, ModalMode::Diamond, &[0.8], &[10, 11], 99);
        assert!(sc.is_some() && sc.clone().unwrap().canonical_id.is_some());
        let canonical = sc.unwrap().canonical_id.unwrap();
        assert_eq!(cache.expand_shortcut(canonical), Some(&[10, 11][..]));
    }
}
