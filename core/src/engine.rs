...
impl Engine {
    ...
    pub fn execute(&mut self, query: Query, node_pool: Vec<ArgNode>, edge_pool: Vec<ArgEdge>) -> QueryResult {
        ...
        // Schedule
        let ready_nodes: Vec<NodeId> = search.graph.node_indices()
            .map(|i| search.graph[i].id).collect();
    ...
    pub fn execute_passage(
        ...
        // Schedule
        let ready_nodes: Vec<NodeId> = final_graph.node_indices()
            .map(|i| final_graph[i].id).collect();
        let _schedule = build_schedule(&final_graph, Some(trd), &self.perf, &ready_nodes);
